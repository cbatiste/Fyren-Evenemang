...
function PageLogo() {
  return (
    <motion.div className={"flex flex-row self-center pt-6 select-none"} initial={{opacity: 0}}
      ...
      <h3 className={"py-12 pr-4 sm:pr-6 md:py-20 md:px-10 text-2xl sm:text-3xl font-bold"}>FYREN</h3>
      <div className={"md:w-[192px] md:h-[302px] w-[128px] h-[201px] relative"}>
        <Image src={"/Fyren-Display-Logo.png"} fill={true} alt={"Fyren logo"} className={"object-contain w-full"}
               priority/>
      </div>
      <h3 className={"py-12 pl-4 sm:pl-6 md:py-20 md:px-10 text-2xl sm:text-3xl font-bold"}>ONLINE</h3>
    </motion.div>
  );
}
  ...
  return (
    <main className={`flex min-h-screen flex-col text-zinc-900`}>
      <section className={'flex flex-col mb-64 w-full'}>
        <PageLogo/>
        <div className={'flex flex-col pt-24 px-8 sm:px-12 lg:px-48'}>
